...
from . import helpers # Itovi(num), __reserve(), __cdp(user, cdp_id), __get(lookup), __get_cdp_debt(cl, cdp_addr)

def open_cdp(address, client, total_malgs, GARD, account_id):
    
    # V1 values
    curr_price = helpers.__get("current_price")
    validator_id = helpers.__get("validator_id")
    gard_id = helpers.__get("gard_id")
    fee_id = helpers.__get("open_fee_id")
    devfee_address = helpers.__get("devfee_address")
    ...
    program = helpers.__cdp(address, account_id)
...
def close_cdp_fee(usr_addr, client, account_id, debt):

    # V1 values
    validator_id = helpers.__get("validator_id")
    gard_id = helpers.__get("gard_id")
    fee_id = helpers.__get("close_fee_id")
    devfee_addr = helpers.__get("devfee_address")
    ...
    program = helpers.__cdp(usr_addr, account_id)
    compiled = compileTeal(program, Mode.Signature, version=6)
    response = client.compile(compiled)
    ...
    # Get current values
    debt = helpers.__get_current_debt(client, contract_addr)    
    curr_price = helpers.__get("current_price")
